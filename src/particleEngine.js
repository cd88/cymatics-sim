export function createParticleEngine({ controls, state, pctx, particleCanvas, waveAt }) {
  ...
  function updateParticles(dt) {
    ...
    const dampingStrength = (pdamp - pdampMin) / Math.max(pdampMax - pdampMin, 1e-6);
    // Higher slider values should feel noticeably more damped at the top end.
    const particleRetention = Math.max(0, (pdampMin + pdampMax) - pdamp - surfDamping - dampingStrength * 0.12);
